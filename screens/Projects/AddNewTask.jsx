...
// //         const membersAssignedToProject = members.filter(member =>
// //   Array.isArray(member.assignedProjects) &&
// //   member.assignedProjects.some(pid => String(pid) == projectId)
// // );
// // console.log("project id",projectId);
// //        console.log("Listtttttttttttttttttttttt",membersAssignedToProject)
// //         if (mounted) {
...
// //     }
// //     if (!assignedTo) {
// //       Alert.alert('Validation', 'Please assign the task to someone.');
// //       return false;
// //     }
...
// //                       {members.find((m) => (m._id || m.id) === assignedTo)?.name || 'Select member'}
// //                     </Text>
...
// //                         <TouchableOpacity
// //                           key={id}
// //                           style={[styles.memberItem, assignedTo === id && styles.memberItemActive]}
// //                           onPress={() => setAssignedTo(id)}
// //                         >
// //                           <Text style={[styles.memberText, assignedTo === id && styles.memberTextActive]}>
// //                             {m.name || m.fullName || `${m.firstName || ''} ${m.lastName || ''}`.trim() || id}
// //                           </Text>
// //                           <Text style={{ fontSize: 12, color: '#6b7280' }}>{m.email}</Text>
// //                         </TouchableOpacity>
// //                       );
// //                     })}
// //                   </View>
...
//         // FIXED: Filter the fetched 'list' (not empty 'members' state)
//         const membersAssignedToProject = list.filter(member =>
//           Array.isArray(member.assignedProjects) &&
//           member.assignedProjects.some(pid => String(pid) == String(projectId)) // String() for safety
//         );
...
//         if (mounted) {
//           setMembers(membersAssignedToProject);
//           // FIXED: Set firstId from filtered list (not full list)
...
//       mounted = false;
//     };
//   }, [projectId]); // FIXED: Depend on projectId to refetch when it changes
...
const PRIORITY_CONFIG = {
  low: {
    ...
    bgColor: '#D1FAE5',
    icon: 'arrow-down',
    description: 'Minor impact, can be deferred'
  },
  medium: {
    label: 'Medium',
    ...
    bgColor: '#FEF3C7',
    icon: 'minus',
    description: 'Normal priority, typical timeline'
  },
  high: {
    label: 'High',
    color: '#EF4444', // Red
...
const AddNewTask = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    startDate: new Date().toISOString(),
    dueDate: new Date(Date.now() + 7 * 24 * 3600 * 1000).toISOString(),
    status: 'todo', // todo | inprogress | blocked | review | done
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const body = {
        ...
        dueDate: formData.dueDate,
        status: formData.status,
        attachments: attachments.filter(a => a.url).map(a => ({ url: a.url, name: a.name })),
      };
  ...
  const TaskTypeButton = ({ value, label }) => (
    <TouchableOpacity
      style={[styles.statusBtn, formData.taskType === value && styles.statusBtnActive]}
      onPress={() => setFormData((p) => ({ ...p, taskType: value }))}
    >
      <Text style={[styles.statusBtnText, formData.taskType === value && styles.statusBtnTextActive]}>{label}</Text>
    </TouchableOpacity>
  );
    ...
    return (
      <TouchableOpacity
        style={[
          ...
          isSelected && [styles.priorityOptionSelected, { borderColor: config.color }],
        ]}
        onPress={() => handlePrioritySelect(priority)}
      >
        <View style={styles.priorityOptionContent}>
          <View style={[styles.priorityIconContainer, { backgroundColor: config.bgColor }]}>
            <Feather 
              name={config.icon} 
              size={20} 
              color={config.color} 
          ...
          <View style={styles.priorityTextContainer}>
            <Text style={[styles.priorityLabel, isSelected && { color: config.color }]}>
              {config.label}
            </Text>
          ...
          <View style={styles.contentContainer}>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.buttonRow}>
                <TaskTypeButton value="annotation" label="Annotation" />
                <TaskTypeButton value="site" label="Site" />
                <TaskTypeButton value="documentation" label="Documentation" />
                <TaskTypeButton value="approval" label="Approval" />
                <TaskTypeButton value="general" label="General" />
              </View>
            </View>
            ...
            {/* <View style={styles.inputGroup}>
              ...
              <View style={styles.selectedPriorityContainer}>
                <View style={[
                  styles.selectedPriorityBadge,
                  { backgroundColor: PRIORITY_CONFIG[formData.priority].bgColor }
                ]}>
                  <Feather 
                ...
                <Text style={styles.selectedPriorityDescription}>
                  {PRIORITY_CONFIG[formData.priority].description}
                </Text>
              </View>
              ...
              <View style={styles.priorityOptionsContainer}>
                ...
                <PriorityOption priority="critical" />
              </View>
            </View> */}
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.buttonRow}>
                <StatusButton value="todo" label="To do" />
                <StatusButton value="inprogress" label="In progress" />
                <StatusButton value="blocked" label="Blocked" />
                <StatusButton value="review" label="Review" />
                <StatusButton value="done" label="Done" />
              </View>
            </View>
            ...
            <View style={styles.inputGroup}>
              ...
              <TouchableOpacity 
                ...
                disabled={attachments.some((a) => a.isUploading)}
              >
                <Ionicons name="cloud-upload-outline" size={48} color="#9CA3AF" />
                <Text style={styles.uploadText}>
                  {attachments.some((a) => a.isUploading) ? 'Uploading...' : 'Choose files to upload'}
