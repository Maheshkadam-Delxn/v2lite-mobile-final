...
const [isUploading, setIsUploading] = useState(false);
  ...
  // Color theme matching your header (blue theme)
  const themeColors = {
    primary: '#2563EB', // Blue color from your header
    primaryLight: '#DBEAFE',
...
const uploadToCloudinary = async (imageUri, fileType = 'image') => {
  ...
  // const addPhoto = () => {
  //   // This would integrate with camera/gallery
...
const addPhoto = async () => {
  ...
  try {
    ...
    if (uploadResult.success) {
      ...
      updateField('photos', newPhotos);
      alert('Success: Photo uploaded to Cloudinary successfully!');
    } else {
...
useEffect(() => {
  ...
  // ⛔ HARD STOP if null / undefined
  if (!survey) return;

  setFormData(prev => ({
    ...prev,

    siteName: survey.location?.siteName ?? '',
    addressLine1: survey.location?.addressLine1 ?? '',
    addressLine2: survey.location?.addressLine2 ?? '',
    city: survey.location?.city ?? '',
    state: survey.location?.state ?? '',
    pincode: survey.location?.pincode ?? '',
    latitude: survey.location?.latitude ?? '',
    longitude: survey.location?.longitude ?? '',
    landmark: survey.location?.landmark ?? '',

    plotShape: survey.plotDetails?.plotShape ?? '',
    plotLength: survey.plotDetails?.plotLength ?? '',
    plotWidth: survey.plotDetails?.plotWidth ?? '',
    plotArea: survey.plotDetails?.plotArea ?? '',
    areaUnit: survey.plotDetails?.areaUnit ?? 'sq.ft',

    setbackFront: survey.setbacks?.setbackFront ?? '',
    setbackBack: survey.setbacks?.setbackBack ?? '',
    setbackLeft: survey.setbacks?.setbackLeft ?? '',
    setbackRight: survey.setbacks?.setbackRight ?? '',
    setbackUnit: survey.setbacks?.setbackUnit ?? 'ft',

    northType: survey.surroundings?.north?.type ?? '',
    northDescription: survey.surroundings?.north?.description ?? '',

    reviewStatus: survey.review?.status ?? 'draft',
    reviewRemarks: survey.review?.remarks ?? '',
 ...
 const handleSaveDraft = () => {
  ...
  try {
    ...
    // Transform formData to match the backend schema
    const transformedData = {
      ...
      measurements: measurements || '', // Include the measurements array
    ...
    console.log("Submitting Survey:", transformedData);
...
navigation.setParams({
  siteSurveyData: transformedData,
});
