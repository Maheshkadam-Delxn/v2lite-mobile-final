    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F5F5F5' }}>
      <ScrollView
        ...
        <View
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 }}>
            <Stat label="BALANCE" value={`+ ${calculateBalance(transactions)} QAR`} color="#000" />
...
const calculateIncoming = (list) =>
  list.filter((t) => t.type.includes('in')).reduce((sum, t) => sum + (Number(t.amount) || 0) * INR_TO_QAR_RATE, 0).toFixed(2);
const calculateOutgoing = (list) =>
  list.filter((t) => t.type.includes('out')).reduce((sum, t) => sum + (Number(t.amount) || 0) * INR_TO_QAR_RATE, 0).toFixed(2);
const calculateBalance = (list) => (calculateIncoming(list) - calculateOutgoing(list)).toFixed(2);
