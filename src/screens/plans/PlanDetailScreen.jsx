...
/**
 * PlanDetailScreen - Dynamic detail view for a single plan
 * Fetches from /api/plans/[id]
 */
...
export default function PlanDetailScreen({ route }) {
  ...
  useEffect(() => {
    ...
    const fetchPlan = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE}/api/plan/${planId}`, { // Replace with your Next.js URL
          headers: {
        ...
        if (result.success) {
          ...
          const mappedData = {
            projectId: apiData.projectId?._id || apiData.projectId,
            name: apiData.name,
            finalizedBy: {
              consultant: "Ahmed Hassan (Consultant)", // Derive/adjust from apiData
              siteManager: "Mohamed Ali",
              date: apiData.updatedAt ? new Date(apiData.updatedAt).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
            },
            measurements: apiData.measurements || [
              { label: "Plot Area", value: "450 sq.m" },
              // Add more defaults or derive from model
            ],
            photos: apiData.photos || [],
            boqSnapshot: apiData.boqSnapshot || {
              totalItems: 42,
            ...
            budget: apiData.budget || { baseline: 35000000, revised: 38200000, currency: "QAR" },
            documents: apiData.documents || [],
            approvalStatus: apiData.approvalStatus || { consultantApproved: true, siteManagerApproved: true, clientApproved: false },
          };
          setData(mappedData);
          setClientApproved(mappedData.approvalStatus.clientApproved);
        } else {
          throw new Error(result.message || "Failed to fetch plan");
        }
      } catch (err) {
        setError(err.message);
        Alert.alert("Error", `Failed to load plan: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 bg-slate-50 justify-center items-center">
        ...
        <Text className="mt-2 text-slate-600">Loading plan details...</Text>
      </SafeAreaView>
    );
  }
  ...
  if (error || !data) {
    return (
      <SafeAreaView className="flex-1 bg-slate-50 justify-center items-center">
        ...
        <Text className="mt-2 text-slate-600 text-center">{error || "No data available"}</Text>
      </SafeAreaView>
    );
  }
  ...
  const onApproveByClient = async () => {
    // TODO: PUT to API for persistence
  ...
  return (
    <View className="flex-1 bg-slate-50 mt-2">
      <ScrollView 
        className="flex-1"
        contentContainerStyle={{ paddingTop: 4, paddingHorizontal: 16, paddingBottom: 40 }}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View className="w-64 mr-3 rounded-2xl overflow-hidden bg-white shadow-md shadow-black/10">
              <Image source={{ uri: item.source }} className="w-full h-48" />
        ...
        <View className="bg-white rounded-2xl p-5 shadow-sm shadow-black/5">
          ...
          {!clientApproved ? (
            <View className="space-y-2.5 mt-4">
              ...
              <TouchableOpacity 
                className="rounded-xl py-4 items-center border border-slate-300"
                onPress={onRequestChanges}
              >
                <Text className="text-slate-700 text-base font-semibold">Request Changes</Text>
              </TouchableOpacity>
            </View>
          ) : (
            <View className="bg-emerald-100 rounded-xl p-4 flex-row items-center mt-4">
